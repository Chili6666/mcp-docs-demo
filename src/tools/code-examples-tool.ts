...
const CodeExamplesParamsSchema = z.object({
  useCase: z.string().describe('The use case or topic for FusionKit code examples. Can be one of the predefined cases: "getting-started", "microfrontend-setup", "authentication", "configuration", "service-integration", or describe what you want to see in natural language (e.g., "how to use fusionkit with react", "fusionkit basic setup", "fusionkit user login", "fusionkit integration examples")'),
  framework: z.enum(['angular', 'react', 'vue', 'vanilla']).optional().default('react').describe('Framework for the code examples: "angular", "react", "vue", or "vanilla". Defaults to "react" if not specified')
});
...
export const getCodeExamplesTool = {
  ...
  description: 'Get FusionKit code examples for different use cases and frameworks. When user asks for FusionKit code examples, setup guides, authentication examples, or framework-specific implementations, extract the use case and framework and pass them as parameters.',
